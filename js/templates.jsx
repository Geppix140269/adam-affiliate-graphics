/*
 * ADAMftd Partner Kit — 15 asset templates.
 * Visual design is APPROVED. Do not redesign — see brief.
 *
 * Non-ASCII characters are declared as named constants via \u escapes so
 * any tooling that may re-encode this file (PowerShell, Windows editors,
 * legacy CI) cannot mangle them.
 */
(function () {
  const { useRef } = React;
  ...
  function Lockup({ width, inset = false, padding = 24, radius = 14, bg = PALETTE.paper, shadow = false }) {
    const img = (
      <img src={LOGO_SRC} alt="ADAMftd Affiliate Programme"
  ...
  function CodeBlock({ aff, dark = true, size = 'md', align = 'left' }) {
    const sizes = {
      ...
      <div style={{
        ...
        <div style={{
          ...
          <span style={{
            ...
            borderRadius: 999, background: PALETTE.teal, display: 'inline-block',
          }} />
    ...
    if (prominence === 'caption') {
      return (
        <div style={{
          ...
          Referred by <span style={{ color: strong, fontWeight: 500 }}>{name}</span>
        </div>
      );
    }
    return (
      <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
        <div style={{
          fontFamily: TYPE.mono, fontSize: 11, letterSpacing: '0.24em',
          textTransform: 'uppercase', color: muted, fontWeight: 500,
        }}>Referred by</div>
        <div style={{
          fontFamily: TYPE.sans, fontSize: 22, fontWeight: 600,
          color: strong, letterSpacing: '-0.01em',
        }}>{name}</div>
      </div>
    );
  }

  function Eyebrow({ children, accent = PALETTE.teal, size = 13, color }) {
    return (
      <div style={{
        fontFamily: TYPE.mono, fontSize: size, letterSpacing: '0.22em',
        textTransform: 'uppercase', color: color || accent, fontWeight: 500,
        display: 'flex', alignItems: 'center', gap: 10,
      }}>
        <span style={{ width: 16, height: 1, background: color || accent, display: 'inline-block' }} />
        {children}
      </div>
    );
  }
  ...
  function LinkedInBanner({ aff, dark = true, line }) {
    ...
    const headline = line || POSITIONING_LINES[0];
    return (
      <div style={{ width: 1584, height: 396, background: bg, position: 'relative', overflow: 'hidden', fontFamily: TYPE.sans, color: fg }}>
        ...
        <div style={{ position: 'absolute', inset: '56px 72px 40px 72px', display: 'grid', gridTemplateColumns: '280px 1px 1fr', gap: 44, alignItems: 'stretch' }}>
          <div style={{ display: 'flex', flexDirection: 'column', justifyContent: 'space-between', gap: 14 }}>
            <Lockup width={240} inset={dark} padding={12} radius={10} bg={dark ? PALETTE.paper : 'transparent'} />
          ...
          <div style={{ display: 'flex', flexDirection: 'column', justifyContent: 'space-between', minWidth: 0, gap: 16 }}>
            <div style={{ fontSize: 44, fontWeight: 600, lineHeight: 1.06, letterSpacing: '-0.025em', color: fg, textWrap: 'balance', maxWidth: 900 }}>{headline}</div>
            <CodeBlock aff={aff} dark={dark} size="md" />
          </div>
        </div>
      </div>
    );
  ...
  function InstagramCuriosity({ aff, dark = true }) {
    const bg = dark ? PALETTE.ink : PALETTE.paper;
    const fg = dark ? PALETTE.white : PALETTE.ink;
      ...
      <div style={{ width: 1080, height: 1080, background: bg, position: 'relative', overflow: 'hidden', fontFamily: TYPE.sans, color: fg }}>
        ...
        <div style={{ position: 'absolute', top: 200, left: 80, right: 80, display: 'flex', flexDirection: 'column', gap: 48 }}>
          <div style={{ display: 'flex', alignItems: 'flex-start', gap: 24 }}>
            <div style={{ fontFamily: TYPE.sans, fontSize: 140, fontWeight: 600, lineHeight: 0.75, color: PALETTE.teal, marginTop: -8, letterSpacing: '-0.04em' }}>{LDQUO}</div>
            <div style={{ fontSize: 60, fontWeight: 600, lineHeight: 1.06, letterSpacing: '-0.025em', color: fg, textWrap: 'pretty' }}>
              How do you find verified buyers and suppliers without Bloomberg-tier budgets?
            </div>
          </div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 14, marginLeft: 4 }}>
            <Eyebrow color={PALETTE.teal} size={12}>The answer</Eyebrow>
            <div style={{ fontSize: 30, fontWeight: 500, lineHeight: 1.32, color: muted, maxWidth: 880, letterSpacing: '-0.01em' }}>
              AI-native trade intelligence, grounded in 7B+ source records.
            </div>
          </div>
        </div>
        ...
        <div style={{ position: 'absolute', left: 60, right: 60, bottom: 60, background: panelBg, borderRadius: 18, padding: '24px 36px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 28, boxShadow: dark ? '0 24px 60px rgba(0,0,0,0.35)' : '0 1px 0 rgba(15,27,45,0.06)' }}>
          ...
          <CodeBlock aff={aff} dark={false} size="md" />
        </div>
      </div>
    );
  }

  function InstagramStats({ aff, dark = true }) {
    const bg = dark ? PALETTE.ink : PALETTE.paper;
    const fg = dark ? PALETTE.white : PALETTE.ink;
    const muted = dark ? 'rgba(238,243,250,0.6)' : 'rgba(15,27,45,0.6)';
    const ruleColor = dark ? 'rgba(238,243,250,0.18)' : 'rgba(15,27,45,0.15)';
    ...
    return (
      <div style={{ width: 1080, height: 1080, background: bg, position: 'relative', overflow: 'hidden', fontFamily: TYPE.sans, color: fg }}>
        ...
        <div style={{ position: 'absolute', bottom: 290, left: 80, right: 80, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <ReferredBy name={aff.full_name} dark={dark} prominence="caption" />
          <div style={{ fontFamily: TYPE.mono, fontSize: 12, letterSpacing: '0.2em', textTransform: 'uppercase', color: muted, textAlign: 'right', maxWidth: 520 }}>
            Trade intelligence for SMEs and the consultancies advising them.
          </div>
        </div>
        <div style={{ position: 'absolute', left: 60, right: 60, bottom: 60, background: panelBg, borderRadius: 18, padding: '24px 36px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 28, boxShadow: dark ? '0 24px 60px rgba(0,0,0,0.35)' : '0 1px 0 rgba(15,27,45,0.06)' }}>
  ...
  function EmailSignature({ aff, dark = false }) {
    const bg = dark ? PALETTE.ink : PALETTE.paper;
    const fg = dark ? PALETTE.white : PALETTE.ink;
      ...
      <div style={{ width: 600, height: 120, background: bg, position: 'relative', overflow: 'hidden', fontFamily: TYPE.sans, color: fg, display: 'flex', alignItems: 'stretch' }}>
        ...
        <div style={{ flex: 1, padding: '20px 24px', display: 'flex', flexDirection: 'column', justifyContent: 'center', gap: 6 }}>
          ...
          <div style={{ fontFamily: TYPE.mono, fontSize: 12, color: dark ? 'rgba(238,243,250,0.8)' : 'rgba(15,27,45,0.72)', display: 'flex', alignItems: 'center', gap: 6 }}>
            <span style={{ width: 5, height: 5, borderRadius: 999, background: PALETTE.teal }} />
            adamftd.com/ref/{aff.code}
          </div>
        </div>
      </div>
  ...
  function InstagramStoryCuriosity({ aff, dark = true }) {
    const bg = dark ? PALETTE.ink : PALETTE.paper;
    const fg = dark ? PALETTE.white : PALETTE.ink;
    const muted = dark ? 'rgba(238,243,250,0.6)' : 'rgba(15,27,45,0.6)';
    const panelBg = dark ? PALETTE.paper : PALETTE.white;
    return (
      <div style={{ width: 1080, height: 1920, background: bg, position: 'relative', overflow: 'hidden', fontFamily: TYPE.sans, color: fg }}>
        ...
        <div style={{ position: 'absolute', top: 280, left: 80, right: 80 }}>
          <div style={{ fontFamily: TYPE.sans, fontSize: 200, fontWeight: 600, lineHeight: 0.75, color: PALETTE.teal, letterSpacing: '-0.04em', marginBottom: 12 }}>{LDQUO}</div>
          <div style={{ fontSize: 88, fontWeight: 600, lineHeight: 1.05, letterSpacing: '-0.025em', color: fg, textWrap: 'pretty', maxWidth: 920 }}>
            How do you find verified buyers and suppliers without Bloomberg-tier budgets?
          </div>
        </div>
        <div style={{ position: 'absolute', top: 1180, left: 80, right: 80 }}>
          <Eyebrow color={PALETTE.teal} size={16}>The answer</Eyebrow>
          <div style={{ marginTop: 22, fontSize: 44, fontWeight: 500, lineHeight: 1.28, color: muted, maxWidth: 900, letterSpacing: '-0.01em' }}>
            AI-native trade intelligence, grounded in 7B+ source records.
          </div>
        </div>
  ...
  function InstagramStoryStats({ aff, dark = true }) {
    const bg = dark ? PALETTE.ink : PALETTE.paper;
    const fg = dark ? PALETTE.white : PALETTE.ink;
    const muted = dark ? 'rgba(238,243,250,0.6)' : 'rgba(15,27,45,0.6)';
    const ruleColor = dark ? 'rgba(238,243,250,0.18)' : 'rgba(15,27,45,0.15)';
    ...
    return (
      <div style={{ width: 1080, height: 1920, background: bg, position: 'relative', overflow: 'hidden', fontFamily: TYPE.sans, color: fg }}>
        ...
        <div style={{ position: 'absolute', top: 260, left: 80, right: 80 }}>
          <Eyebrow color={PALETTE.teal} size={16}>The substrate</Eyebrow>
  ...
  function XHeader({ aff, dark = true, line }) {
    const bg = dark ? PALETTE.ink : PALETTE.paper;
    const fg = dark ? PALETTE.white : PALETTE.ink;
    const ruleColor = dark ? 'rgba(238,243,250,0.18)' : 'rgba(15,27,45,0.15)';
    const headline = line || 'AI-native AND source-anchored.';
    return (
      <div style={{ width: 1500, height: 500, background: bg, position: 'relative', overflow: 'hidden', fontFamily: TYPE.sans, color: fg }}>
        <div style={{ position: 'absolute', top: 0, left: 0, right: 0 }}><GradientHairline h={3} /></div>
        <div style={{ position: 'absolute', inset: '80px 80px 60px 80px', display: 'grid', gridTemplateColumns: '360px 1px 1fr', gap: 48, alignItems: 'stretch' }}>
          <div style={{ display: 'flex', flexDirection: 'column', justifyContent: 'space-between' }}>
            <Lockup width={260} inset={dark} padding={14} radius={10} bg={dark ? PALETTE.paper : 'transparent'} />
            <ReferredBy name={aff.full_name} dark={dark} prominence="emphasis" />
          </div>
          <div style={{ width: 1, background: ruleColor }} />
          <div style={{ display: 'flex', flexDirection: 'column', justifyContent: 'space-between', minWidth: 0 }}>
            <div style={{ fontSize: 60, fontWeight: 600, lineHeight: 1.05, letterSpacing: '-0.03em', color: fg, textWrap: 'balance', maxWidth: 880 }}>{headline}</div>
            <CodeBlock aff={aff} dark={dark} size="lg" />
          </div>
        </div>
      </div>
    );
  ...
  function ShareCard({ aff, dark = true, line }) {
    const bg = dark ? PALETTE.ink : PALETTE.paper;
    const fg = dark ? PALETTE.white : PALETTE.ink;
    const muted = dark ? 'rgba(238,243,250,0.6)' : 'rgba(15,27,45,0.6)';
    const ruleColor = dark ? 'rgba(238,243,250,0.18)' : 'rgba(15,27,45,0.15)';
    ...
    return (
      <div style={{ width: 1600, height: 900, background: bg, position: 'relative', overflow: 'hidden', fontFamily: TYPE.sans, color: fg }}>
        ...
        <div style={{ position: 'absolute', top: 140, left: 80, right: 80, bottom: 270, display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
          <Eyebrow color={PALETTE.teal} size={15}>Trade intelligence, grounded</Eyebrow>
          <div style={{ marginTop: 24, fontSize: 88, fontWeight: 600, lineHeight: 1.02, letterSpacing: '-0.03em', color: fg, textWrap: 'balance', maxWidth: 1280 }}>{headline}</div>
          <div style={{ marginTop: 28, fontSize: 28, fontWeight: 500, color: muted, letterSpacing: '-0.01em', maxWidth: 1100 }}>
            For SMEs, exporters, and the consultancies advising them. Not Bloomberg-tier budgets.
          </div>
        </div>
        <div style={{ position: 'absolute', bottom: 50, left: 80, right: 80, height: 220, paddingTop: 28, borderTop: `1px solid ${ruleColor}`, display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 48, boxSizing: 'border-box' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 28 }}>
            <Lockup width={200} inset={dark} padding={12} radius={10} bg={dark ? PALETTE.paper : 'transparent'} />
  ...
  function FacebookCover({ aff, dark = true }) {
    const bg = dark ? PALETTE.ink : PALETTE.paper;
    const fg = dark ? PALETTE.white : PALETTE.ink;
    const muted = dark ? 'rgba(238,243,250,0.65)' : 'rgba(15,27,45,0.65)';
    const ruleColor = dark ? 'rgba(238,243,250,0.18)' : 'rgba(15,27,45,0.15)';
    return (
      <div style={{ width: 820, height: 312, background: bg, position: 'relative', overflow: 'hidden', fontFamily: TYPE.sans, color: fg }}>
        ...
        <div style={{ position: 'absolute', inset: '36px 40px', display: 'grid', gridTemplateColumns: '220px 1px 1fr', gap: 28, alignItems: 'stretch' }}>
          <div style={{ display: 'flex', flexDirection: 'column', justifyContent: 'space-between' }}>
            <Lockup width={200} inset={dark} padding={10} radius={8} bg={dark ? PALETTE.paper : 'transparent'} />
            <div style={{ fontFamily: TYPE.mono, fontSize: 9, letterSpacing: '0.22em', textTransform: 'uppercase', color: muted }}>
              Referred by <span style={{ color: fg, fontWeight: 500 }}>{aff.full_name}</span>
            </div>
          </div>
          <div style={{ width: 1, background: ruleColor }} />
          <div style={{ display: 'flex', flexDirection: 'column', justifyContent: 'space-between', minWidth: 0 }}>
            <div style={{ fontSize: 26, fontWeight: 600, lineHeight: 1.1, letterSpacing: '-0.02em', color: fg, textWrap: 'balance' }}>
              AI-native trade intelligence, grounded in source data.
            </div>
            <CodeBlock aff={aff} dark={dark} size="sm" />
          </div>
        </div>
      </div>
    );
  ...
  function EmailBanner({ aff, dark = true }) {
    const bg = dark ? PALETTE.ink : PALETTE.paper;
    const fg = dark ? PALETTE.white : PALETTE.ink;
    const ruleColor = dark ? 'rgba(238,243,250,0.18)' : 'rgba(15,27,45,0.15)';
    return (
      <div style={{ width: 1200, height: 400, background: bg, position: 'relative', overflow: 'hidden', fontFamily: TYPE.sans, color: fg }}>
        <div style={{ position: 'absolute', top: 0, left: 0, right: 0 }}><GradientHairline h={3} /></div>
        <div style={{ position: 'absolute', inset: '52px 60px', display: 'grid', gridTemplateColumns: '320px 1px 1fr', gap: 40, alignItems: 'stretch' }}>
          <div style={{ display: 'flex', flexDirection: 'column', justifyContent: 'space-between' }}>
            <Lockup width={240} inset={dark} padding={14} radius={10} bg={dark ? PALETTE.paper : 'transparent'} />
            <ReferredBy name={aff.full_name} dark={dark} prominence="emphasis" />
          </div>
          <div style={{ width: 1, background: ruleColor }} />
          <div style={{ display: 'flex', flexDirection: 'column', justifyContent: 'space-between', minWidth: 0 }}>
            <div>
              <Eyebrow color={PALETTE.teal} size={12}>{'You' + RSQUO + 've been invited'}</Eyebrow>
              <div style={{ marginTop: 14, fontSize: 40, fontWeight: 600, lineHeight: 1.08, letterSpacing: '-0.025em', color: fg, textWrap: 'balance', maxWidth: 720 }}>
                {aff.first_name} thinks ADAMftd can help your trade work.
              </div>
            </div>
            <CodeBlock aff={aff} dark={dark} size="md" />
          </div>
  ...
  function ZoomBackground({ aff, dark = true }) {
    const bg = dark ? PALETTE.ink : PALETTE.paper;
    const fg = dark ? PALETTE.white : PALETTE.ink;
    const muted = dark ? 'rgba(238,243,250,0.55)' : 'rgba(15,27,45,0.55)';
    ...
    return (
      <div style={{ width: 1920, height: 1080, background: bg, position: 'relative', overflow: 'hidden', fontFamily: TYPE.sans, color: fg }}>
        ...
        <div style={{ position: 'absolute', top: 240, left: 100, right: 800 }}>
          <Eyebrow color={PALETTE.teal} size={16}>Trade intelligence, grounded</Eyebrow>
          <div style={{ marginTop: 28, fontSize: 84, fontWeight: 600, lineHeight: 1.02, letterSpacing: '-0.03em', color: fg, textWrap: 'balance' }}>
            AI-native AND source-anchored.
          </div>
          <div style={{ marginTop: 28, fontSize: 24, fontWeight: 500, color: muted, letterSpacing: '-0.005em', maxWidth: 760 }}>
            For SMEs, exporters, and the consultancies advising them.
          </div>
        </div>
        <div style={{ position: 'absolute', left: 100, bottom: 80 }}>
          <Lockup width={340} inset={dark} padding={20} radius={12} bg={dark ? PALETTE.paper : 'transparent'} />
        </div>
        <div style={{ position: 'absolute', top: 80, right: 100, display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: 12 }}>
          <div style={{ fontFamily: TYPE.mono, fontSize: 12, letterSpacing: '0.22em', textTransform: 'uppercase', color: muted }}>
            Referred by <span style={{ color: fg, fontWeight: 500 }}>{aff.full_name}</span>
          </div>
          <div style={{ padding: '18px 24px', background: dark ? 'rgba(238,243,250,0.06)' : 'rgba(15,27,45,0.04)', border: `1px solid ${ruleColor}`, borderRadius: 10 }}>
            <CodeBlock aff={aff} dark={dark} size="md" align="right" />
          </div>
        </div>
      </div>
    );
  ...
  function BusinessCardFront({ aff }) {
    return (
      <div style={{ width: 1050, height: 600, background: PALETTE.paper, position: 'relative', overflow: 'hidden', fontFamily: TYPE.sans, color: PALETTE.ink }}>
        ...
        <div style={{ position: 'absolute', inset: '60px 64px', display: 'flex', flexDirection: 'column', justifyContent: 'space-between' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            <Eyebrow color={PALETTE.teal} size={11}>Trade intelligence, grounded</Eyebrow>
            <div style={{ fontSize: 32, fontWeight: 600, letterSpacing: '-0.02em', lineHeight: 1.15, maxWidth: 760, textWrap: 'balance', color: PALETTE.ink }}>
              AI-native AND source-anchored.
            </div>
          </div>
        </div>
    ...
    return (
      <div style={{ width: 1050, height: 600, background: PALETTE.ink, position: 'relative', overflow: 'hidden', fontFamily: TYPE.sans, color: PALETTE.white }}>
        ...
        <div style={{ position: 'absolute', inset: '60px 64px', display: 'flex', flexDirection: 'column', justifyContent: 'space-between' }}>
          ...
          <CodeBlock aff={aff} dark={true} size="lg" />
        </div>
      </div>
    );
  }

  function A4OnePager({ aff }) {
      ...
      <div style={{ width: 794, height: 1123, background: PALETTE.paper, position: 'relative', overflow: 'hidden', fontFamily: TYPE.sans, color: PALETTE.ink }}>
        ...
        <div style={{ position: 'absolute', inset: '64px 56px 56px 56px', display: 'flex', flexDirection: 'column', gap: 32 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
            <Lockup width={300} />
            <div style={{ fontFamily: TYPE.mono, fontSize: 9, letterSpacing: '0.24em', textTransform: 'uppercase', color: muted, textAlign: 'right' }}>
              {'One-pager ' + MIDDOT + ' v1'}<br />May 2026
            </div>
          </div>
          <div style={{ height: 1, background: ruleColor }} />
          <div>
            <Eyebrow color={PALETTE.teal} size={11}>Trade intelligence, grounded</Eyebrow>
            <div style={{ marginTop: 14, fontSize: 40, fontWeight: 600, lineHeight: 1.08, letterSpacing: '-0.025em', color: PALETTE.ink, textWrap: 'balance', maxWidth: 640 }}>
              AI-native trade intelligence, grounded in source data.
            </div>
            <div style={{ marginTop: 14, fontSize: 16, lineHeight: 1.5, color: muted, maxWidth: 600 }}>
              For SMEs, exporters, and the consultancies advising them. A conversational AI that talks to verified trade data, not a search box.
            </div>
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', borderTop: `1px solid ${ruleColor}`, borderBottom: `1px solid ${ruleColor}` }}>
            {[
              { v: '7B+', l: 'Verified trade records' },
              { v: '87', l: 'Platform capabilities' },
              { v: '5', l: 'Intelligence layers' },
            ].map((s, i) => (
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 28 }}>
            {[
              ...
              <div key={c.h}>
                <div style={{ fontFamily: TYPE.mono, fontSize: 10, letterSpacing: '0.22em', textTransform: 'uppercase', color: PALETTE.teal, fontWeight: 500, marginBottom: 10 }}>{c.h}</div>
          ...
          <div style={{ background: PALETTE.ink, color: PALETTE.white, borderRadius: 14, padding: '28px 32px', display: 'grid', gridTemplateColumns: '1fr 1px 1fr', gap: 28, alignItems: 'center' }}>
            ...
            <CodeBlock aff={aff} dark={true} size="md" />
          </div>
        </div>
      </div>
    );
  ...
  function Artboard({ asset, aff, dark, line, scale }) {
